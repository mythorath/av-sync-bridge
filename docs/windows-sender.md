...
Replace the uppercase placeholders with the explicitly selected destination and three distinct ports. All run options are mandatory. `--seconds` is an overall observation deadline of 1–30 seconds, including clock acquisition and any pipeline rebuilds; retries do not extend it. Help, no arguments, and invalid arguments open no endpoint and send no network traffic. Only numeric unicast IPv4 destinations are accepted in this version.
...
```text
WASAPI packet with capture PTS
  -> bounded appsrc
  -> audioconvert with explicit normalized stereo matrix
  -> stereo float at the original sample rate
  -> audioresample, quality 10 / Kaiser / automatic sinc table
  -> 48 kHz stereo S24BE
...
The sender's pipeline uses base time zero and start time `GST_CLOCK_TIME_NONE`. Appsrc has `do-timestamp=false` and receives mapped capture timestamps. There is **no sender-side presentation delay** and no manual offset for resampler processing latency. Resampling produces its own output timestamps from the input anchor and output sample count; the RTP/RTCP correspondence must be validated against those actual converted output timestamps, not an unchanged copy of input metadata.
...
| RTCP correspondence | `rtcp-sync-send-time=false`: capture/media time, not transmission latency |
...
These are fail-visible prototype policies, not proven seamless recovery. In particular, **fixed-rate `audioresample` is not adaptive device-clock correction**. A sound device can drift against the shared clock even when network timing is accurate. Long-run ASRC/rate-control behavior remains an unimplemented, independently measured milestone; this short sender does not solve it by itself.
